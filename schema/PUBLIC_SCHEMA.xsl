...
　　<xsl:for-each select="module">
   ...
   <tr bgcolor="#d7e2da">
   <td align="center"><b>模块名称：<font color="red"><xsl:value-of select="@chineseDescription"/>(<xsl:value-of select="@name"/>)</font></b></td>
   </tr>
   </table>
   <TABLE width="90%" cellspacing="1" cellpadding="1" ALIGN="CENTER">
     <THEAD>
      ...
      <xsl:for-each select="table">
       ...
       <h5 ALIGN="left">
        <xsl:for-each select="rem">
         <font color="#660000"><xsl:value-of select="."/><p/></font>
        </xsl:for-each>
       </h5>
       <H5 ALIGN="CENTER">
        <b><a style="font-size: 18px"><xsl:attribute name="name"><xsl:value-of select="@name"/></xsl:attribute>
        <xsl:value-of select="@chineseDescription"/>
         (<xsl:value-of select="@name"/>)</a></b><br/>
         package:<xsl:value-of select="@pkg"/>　　jspPath:<xsl:value-of select="@jspPath"/>　　<a href="#main">返回</a>
       </H5>
